(function (global, factory) {
  ...
  */
  ...
  var Draw = /*#__PURE__*/function () {
    function Draw($targetElem, options) {
      var _this = this;
      ...
      if (o.toolbar) {
        var toolList = o.toolbar.trim().replace(/\s{2,}/g, ' ').split(' ');
        ...
        toolList.forEach(function (tool) {
          ...
          if (tool === 'fontSize') {
            ...
            o.fontSize.list.forEach(function (size) {
            ...
            var obj = _this._createColorPicker(function (color) {
              $elem.style.backgroundColor = color;
              _this.options.style.strokeStyle = color;
              var $textarea = _this.textarea.$textarea;
            ...
            _this.color = Object.assign(_this.color, obj);
            $elem.appendChild(_this.color.$colorPiker);
          } else {
          ...
          addClass($elem, "".concat(o.className, "-tool-").concat(tool));
          $tool.appendChild($elem);

          _this._addEvent($elem, tool);
        });
      }

      addClass($targetElem, o.className);
      addClass($container, "".concat(o.className, "-container"));
      addClass($tool, "".concat(o.className, "-tool"));
      addClass($board, "".concat(o.className, "-board"));
      addClass($img, "".concat(o.className, "-img"));
      addClass($canvas, "".concat(o.className, "-canvas"));
      addClass($previewCanvas, "".concat(o.className, "-canvas-preview"));
    ...
    _createClass(Draw, [{
      key: "input",
      value: function input(background) {
        var $img = this.$img;
      ...
      value: function output() {
        ...
        return new Promise(function (resolve, reject) {
          ...
          $img.src = _this2.$canvas.toDataURL();
        });
      }
    }, {
      key: "_addEvent",
      value: function _addEvent($elem, type) {
        var _this3 = this;

        var $board = this.$board;
        var $tool = this.$tool;
        var method = type === 'fontSize' ? 'change' : 'click';
        $elem.addEventListener(method, function (e) {
          if (type === 'fontSize') {
            var size = e.target.value;
            var $textarea = _this3.textarea.$textarea;
            _this3.options.fontSize.value = size;

            if ($textarea) {
              $textarea.style.fontSize = size + 'px';
            }
          } else if (type === 'color') {
            _this3._toggleStatus(_this3.color.$colorPiker);
          } else {
            if (hasClass($elem, 'active')) {
              removeClass($elem, 'active');
              _this3.type = '';
            } else {
              var $lastActiveBtn = Array.prototype.filter.call($tool.querySelectorAll('button'), function (child) {
                return hasClass(child, 'active');
              })[0];
            ...
            _this3._clearPreviewCanvas();

            _this3._togglePreviewCanvas();

            _this3._toggleStatus(_this3.color.$colorPiker, false);

            _this3._toggleStatus(_this3.textarea.$textarea, type === 'textarea' && hasClass($elem, 'active'));

            _this3.textarea.reset && _this3.textarea.reset();
          }
        });
      }
    }, {
      key: "_isShowPreview",
      value: function _isShowPreview() {
        return ['circle', 'rectangle'].includes(this.type);
      }
    }, {
      key: "_addCanvasEvent",
      value: function _addCanvasEvent() {
        var that = this;
        var $canvas = this.$canvas;
        var $previewCanvas = this.$previewCanvas;
        var ctx = this.ctx;
        ...
        function mousedown(e) {
          ...
          var isShowPreview = that._isShowPreview();
          ...
          that["_".concat(that.type)](isShowPreview ? previewCtx : ctx, x, y, isShowPreview);
        ...
        function handleMouseUp(e) {
          that["_".concat(that.type)](ctx, e.offsetX, e.offsetY, false);
        }
      ...
      key: "_eraser",
      value: function _eraser(ctx, x, y) {
        ctx.save();
        ctx.beginPath();
      ...
      key: "_circle",
      value: function _circle(ctx, x, y, isShowPreview) {
        var startPoint = this.startPoint;
        var centerPointX = (x + startPoint.x) / 2;
        var centerPointY = (y + startPoint.y) / 2;
        var distance = getDistance(x, startPoint.x, y, startPoint.y);
      ...
      key: "_rectangle",
      value: function _rectangle(ctx, x, y, isShowPreview) {
        var startPoint = this.startPoint;
        var width = Math.abs(startPoint.x - x);
        var height = Math.abs(startPoint.y - y); // 计算左上角位置

        var startX = startPoint.x < x ? startPoint.x : x;
        var startY = startPoint.y < y ? startPoint.y : y;
      ...
      key: "_clearPreviewCanvas",
      value: function _clearPreviewCanvas() {
        this.previewCtx.clearRect(0, 0, this.$previewCanvas.width, this.$previewCanvas.height);
      }
    }, {
      key: "_togglePreviewCanvas",
      value: function _togglePreviewCanvas() {
        this.$previewCanvas.style.display = this._isShowPreview() ? 'block' : 'none';
      }
    }, {
      key: "_setStyle",
      value: function _setStyle(ctx) {
        var obj = this.options.style;
      ...
      value: function _createColorPicker(callback) {
        var _this4 = this;

        var activeColor = this.options.style.strokeStyle;
        ...
        $div.addEventListener('click', function (e) {
          ...
          if ($elem && $elem.nodeName.toUpperCase() === 'SPAN') {
            if (!hasClass($elem, 'active')) {
              var $lastActiveElem = Array.prototype.filter.call($div.querySelectorAll('span'), function (child) {
                return hasClass(child, 'active');
              })[0];
      ...
      key: "_toggleStatus",
      value: function _toggleStatus($elem, status) {
        if ($elem) {
          if (status === undefined) {
      ...
      value: function _createTextarea() {
        ...
        reset();
        addClass($textarea, "".concat(o.className, "-textarea"));
        addClass($box, "".concat(o.className, "-textarea-box"));
        addClass($text, "".concat(o.className, "-textarea-text"));
        addClass($border, "".concat(o.className, "-textarea-border"));
        addClass($sure, "".concat(o.className, "-textarea-sure"));
        addClass($cancel, "".concat(o.className, "-textarea-cancel"));
        ...
        $sure.addEventListener('click', function () {
          ...
          html2canvas($text, {
            ...
            img.onload = function () {
              _this5.ctx.drawImage(img, $box.offsetLeft, $box.offsetTop, width, height);
        ...
        $cancel.addEventListener('click', function () {
          _this5.$tool.querySelector('[type=textarea]').click();
        });
